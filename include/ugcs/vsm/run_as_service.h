...
/** Result of Run_as_service function
 * SERVICE_RESULT_OK = 0:
 *   Was invoked as service and is about to end. (service has stopped).
 *   - OR -
 *   Service specific parameters processed successfully.
 *   Application should terminate normally on this return code.
 *
 * SERVICE_RESULT_ERROR:
 *   Error while Service specific parameters processing.
 *   Application should terminate with error on this return code.
 *
 * SERVICE_RESULT_NORMAL_INVOCATION -1:
 *   Was not invoked as service AND no --service* parameters given.
 *   Application should continue as if launched normally (not as service).
 */
typedef enum {
    SERVICE_RESULT_OK = 0,                  // must be zero.
    SERVICE_RESULT_ERROR = 1,               // must be greater than zero
                                            // More error codes can be added here.
    SERVICE_RESULT_NORMAL_INVOCATION = -1   // must be less than zero.
 ...
 * returns only when the service is stopped. The return value is SERVICE_RESULT_OK.
 *
