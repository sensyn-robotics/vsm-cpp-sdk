...
#define dlx_max(a, b)  (((a) > (b)) ? (a) : (b))
#define dlx_min(a, b)  (((a) < (b)) ? (a) : (b))
	...
	case RCHART('['):
		{
			...
			if( flags & IGNORECASE )
			{
				...
				for(i=0; i<oldcount; i++)
				{
					...
					{
						newmin = tolower( dlx_max(RCHART('A'), ranges[i*2  ]) );
						newmax = tolower( dlx_min(RCHART('Z'), ranges[i*2+1]) );
					...
					if( ranges[i*2] <= RCHART('z') && ranges[i*2+1] >= RCHART('a') )
					{
						newmin = toupper( dlx_max(RCHART('a'), ranges[i*2  ]) );
						newmax = toupper( dlx_min(RCHART('z'), ranges[i*2+1]) );
