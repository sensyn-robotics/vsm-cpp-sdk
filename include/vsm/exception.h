...
#define VSM_SYS_EXCEPTION(__msg, ...) do { \
    std::string sys_msg = vsm::Log::Get_system_error(); \
    VSM_EXCEPTION(vsm::System_exception, __msg ": %s", ## __VA_ARGS__, sys_msg.c_str()); \
} while (false)
...
#define VSM_DEFINE_EXCEPTION(__exc_class, ...) \
    ...
    typedef vsm::Param_exception<__exc_class ## _dummy_struct, ## __VA_ARGS__> __exc_class
...
#define VSM_DEFINE_DERIVED_EXCEPTION(__base_class, __exc_class) \
    ...
    typedef vsm::Derived_exception<__base_class, __exc_class ## _dummy_struct> __exc_class
