...
public:
    ...
    public:
        ...
        class Native_handle {
        public:
            /** Helper class for passing native handle between base and derived
             * processor classes.
             */
            class Envelope {
            public:
                /** Get underlying platform handle.
                 * @return Value which is interpreted differently depending on
                 *      target platform. Logically is a pointer to underlying
                 *      platform file handle.
                 *      Unix: points to file descriptor value.
                 *      Windows: points to file handle value of HANDLE type.
                 */
                virtual void *
                Get_handle() = 0;
            };
            ...
            /** Platform-dependent instantiation method. Should create an
             * instance of platform-dependent derived class.
             *
             * @param path File path.
             * @param mode Parsed file opening mode.
             * @param handle Envelope with pre-created native file handle if
             *      any, nullptr if it should be opened.
             * @param write_handle Separate handle for writing, if any.
             * @return Created instance.
             */
            static std::unique_ptr<Native_handle>
            Create(const std::string &path, Mode mode,
                   Stream::Native_handle::Envelope *handle = nullptr,
                   Stream::Native_handle::Envelope *write_handle = nullptr);
    ...
    protected:
        /** Construct file stream. This is protected constructor for derived
         * classes which want to create native handle and wrap it into base class.
         *
         * @param handle Envelope with platform native handle created by a
         *      derived class. The handle used either for both reading and
         *      and writing (if no write_handle specified) or only for reading.
         * @param write_handle Envelope with platform native handle created by a
         *      derived class. The handle is used for write operations.
         * @param processor Associated processor instance.
         * @param path File path.
         * @param mode Parsed file opening mode.
         * @param maintain_pos Indicates whether file position maintenance is
         *      required. See {@link File_processor::Open}.
         */
        Stream(Native_handle::Envelope *handle,
               Native_handle::Envelope *write_handle,
               File_processor::Ptr processor,
               const std::string &path, Mode mode, bool maintain_pos);
    private:
        ...
        std::unique_ptr<Native_handle> native_handle;
