...
public:
    ...
    virtual const std::map<MESSAGE_ID_TYPE, uint8_t> *
    Get_crc_extra_byte_map() const
    {
        return &crc_extra_bytes_map;
    }
private:
    static const Extension instance;
    /** Mapping from Mavlink message id to message extra CRC byte. Filled
     * automatically by generator.
     */
    static const std::map<MESSAGE_ID_TYPE, uint8_t> crc_extra_bytes_map;
};
...
public:
     ...
     */
	Message(uint8_t system_id, uint8_t component_id, Io_buffer::Ptr buffer) :
	    payload(buffer),
...
public:
    ...
    /**
     * Get crc extra byte of a specific Mavlink message type.
     ...
     */
    static uint8_t
    Get_extra_byte(MESSAGE_ID_TYPE message_id, const Extension &ext = Extension::Get());
...
/** Mavlink system id type. Likely to be changed. */
...
struct Header {
    /** Packet start signature, should have @ref vsm::mavlink::START_SIGN value. */
	Uint8 start_sign;
...
#include <vsm/auto_mavlink_enums.h>
#include <vsm/auto_mavlink_messages.h>
