...
{
    struct find_info *data = static_cast<find_info*>(opaque_data);
...
get_bc(struct output_buffer *ob , struct bfd_set *set , const char *procname)
{
    ...
    set->bc = static_cast<bfd_ctx*>(malloc(sizeof(struct bfd_ctx)));
...
_backtrace(struct output_buffer *ob, struct bfd_set *set, int depth , LPCONTEXT context)
{
    ...
    while(StackWalk(IMAGE_FILE_MACHINE_I386,
        ...
        if (module_base &&
            GetModuleFileNameA(reinterpret_cast<HINSTANCE>(module_base), module_name_raw, MAX_PATH)) {
...
static const char * report_base_name;
...
exception_filter(LPEXCEPTION_POINTERS info)
{
    ...
    if (!SymInitialize(GetCurrentProcess(), 0, TRUE)) {
        ...
        bfd_init();
        struct bfd_set *set = static_cast<bfd_set*>(calloc(1,sizeof(*set)));
...
void
backtrace_register(const char* base_name)
{
	report_base_name = base_name;
    if (g_output == NULL) {
        g_output = static_cast<char*>(malloc(BUFFER_MAX));
...
{
    backtrace_register(reports_file_base.c_str());
}
