...
def ParseEnumEntry(e_entry):
    entry = MlEnum.Entry(e_entry.attrib['name'], 
                         int(e_entry.attrib['value']) if 'value' in e_entry.attrib else None)
    for e in e_entry:
...
def GenerateMessageExtraBytes(f):
    f.write('const std::map<MESSAGE_ID_TYPE, uint8_t> mavlink::Extension::crc_extra_bytes_map = {\n')
    
    for msgName in opts.messages:
        msg = opts.messages[msgName]
        if msg.file.namespace is not None:
            continue
        f.write('{MESSAGE_ID::%s, %d},\n' % (msg.name, msg.crcExtraByte))
    ...
    for fileName in opts.files:
        ...
        if file.namespace is None:
            continue
        f.write('const %s::Extension %s::Extension::instance;\n' % (file.namespace, file.namespace))
        f.write('const std::map<MESSAGE_ID_TYPE, uint8_t> %s::Extension::crc_extra_bytes_map = {\n' % file.namespace)
        for msg in file.messages:
            f.write('{%s::MESSAGE_ID::%s, %d},\n' % (file.namespace, 
                                                     msg.name, 
                                                     msg.crcExtraByte))
        f.write('};\n\n')
...
''')
    ...
    f.write('using namespace vsm;\n')
    f.write('using namespace vsm::mavlink;\n\n')
...
def GenerateCppOutput():
    ...
    opts.headersDir = os.path.join(opts.includeDir, 'vsm')
